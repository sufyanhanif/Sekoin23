...
class Rest{
    ...
    public function getPembeli($pblId='') {
        ...
        $pblQuery = "SELECT id, transaction_date, email, name, product_code, qty, payment, total_price, status FROM ".$this->pblTable." $sqlQuery
        ORDER BY id ASC";
    ...
    public function insertPembelian($pblData) {
        ...
        $stmt = $this->dbConnect->prepare("INSERT INTO " . $this->pblTable . " (transaction_date, email, name, product_code, qty, payment, total_price, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssssss", $transactionDate, $email, $name, $productCode, $qty, $payment, $totalPrice, $status);
